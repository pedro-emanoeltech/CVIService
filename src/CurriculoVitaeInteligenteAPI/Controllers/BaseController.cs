...
namespace CurriculoVitaeInteligenteAPI.Controllers
{
        ...
        : ControllerBase, IBaseController<T, TRequestDTO, TResponsesDTO> where T : ClassBase where TRequestDTO : BaseRequest where TResponsesDTO : BaseResponse 
    {
        ...
        public bool ConsiderarContaId = false;
        ...
        public virtual async Task<ActionResult<IQueryable<TResponsesDTO>>> GetList()
        {
            string contaId = string.Empty;
            if (ConsiderarContaId)
            {
                var accessToken = HttpContext.GetTokenAsync("access_token");
                 contaId = _baseServiceApp.GetClaimsFromToken(accessToken.Result!.ToString()).ToString()!;
            }
           
                var result = await _baseServiceApp.GetList(contaId);
        ...
        [HttpPost]
        public virtual async Task<ActionResult<TResponsesDTO>> Add([FromBody] TRequestDTO request)
        {
            try
            {
                if (request == null)
                ...
                var accessToken = HttpContext.GetTokenAsync("access_token");
                var contaId = _baseServiceApp.GetClaimsFromToken(accessToken.Result!.ToString()).ToString()!;
                requestConta.GetType().GetProperty("ContaId")?.SetValue(requestConta, contaId.ToString());
        ...
        [HttpPut("{id}")]
        [Consumes("application/json", new string[] { })]
        public virtual async Task<ActionResult<T>> Edit([FromBody] TRequestDTO request, string id)
        {
            if (request == null)
            {
